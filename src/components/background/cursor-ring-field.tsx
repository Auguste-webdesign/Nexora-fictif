 ...
 *
 * Integrated into Nexora as the hero's technological ground layer. The file is
 * the supplied component, with only the Framer-preset wrapper replaced by a
 * typed default export plus a `className` passthrough so it can be positioned
 ...
 * band (`t2`), and the fill inside (`t3`). Sampling from HOME and not from the
 * live position is what keeps the ring a ring — read the live position and a
 * shoved point carries its own halo with it, smearing the band into a comet.
 *
 ...
 * relaxation, so its equilibrium depends on the decay. Full working in the
 * comment above SIM_FRAG. Only the ring's cursor follow is dt-corrected.
 *
...
const CURSOR_LERP = 0.12 // 130ms time constant
const WANDER_LERP = 0.01 // the resting drift when no pointer is present
...
const RING_EDGE = 4 // frozen cut dial: the thin inner band that does the pushing
...
void fieldTerms(
    ...
    // Two octaves of drift plus a standing wave, scaled by distance to the ring
    // so the band stays coherent while the far field ripples.
...
//
// With k = 0.8 that gain is a fixed 1.25x. Make k depend on frame time and the
// gain moves with it — 1.250 at 60fps, 1.288 at 120fps — so every ordinary late
// frame rescales the ENTIRE field about the origin at once: a ~20px correlated
// radial pulse that reads as depth flicker. The constants stay as written.
const SIM_FRAG = `
precision highp float;
...
// two multiplies and a constant w. Every point sits on the z = 0 plane, which is
// why gl_PointSize carries no attenuation term — there is nothing to attenuate.
...
// centre — that radial alignment is what reads as a shockwave rather than noise.
// The noise term on the angle stops the alignment from being perfect, which
// would band into visible spokes.
const RENDER_FRAG = `
precision highp float;
...
void main(){
    ...
    // index is one. Each pass lerps toward the next stop over that stop's span,
    // CLAMPED and linear, so once p is past a segment its factor is exactly 1
    // and the result is an exact piecewise-linear ramp.
...
// second because WebGL2 makes it colour-renderable in core; WebGL1 float third
// because it needs two separate extensions; static last.
function makeContext(canvas: HTMLCanvasElement) {
    ...
    // zero vertex texture units cannot run the sim whatever the extensions say.
...
function poissonDisk(
    ...
    while (active.length > 0 && px.length < MAX_POINTS) {
        ...
        for (let t = 0; t < tries; t++) {
            ...
            let ok = true
            for (
                let j = Math.max(0, cy - 2);
                j <= Math.min(gh - 1, cy + 2) && ok;
                j++
            ) {
                for (
                    let i = Math.max(0, cx - 2);
                    i <= Math.min(gw - 1, cx + 2);
                    i++
                ) {
                    const q = grid[j * gw + i]
                    if (q < 0) continue
...
export interface CursorRingFieldProps {
    ...
    ring?: RingProps
    /** Global alpha applied in the shader, so the field can sit under content. */
    opacity?: number
    /** Skip the GL work entirely (reduced motion, off-screen sections). */
    paused?: boolean
    className?: string
...
export function CursorRingField({
    ...
    speed = 6,
    cameraDistance = 160,
    ring = { push: 0, width: 9, radius: 12, turbulence: 0 },
    opacity = 1,
    paused = false,
    className = "",
    style,
    ...
    // that the ring lights rather than shoves.
    ...
    // uniform reads as garbage on some drivers.
    const rawPoints = (colors ?? DEFAULT_COLORS).filter(Boolean)
    ...
    // many points exist. It goes through its own ref and a dirty flag.
    ...
    useEffect(() => {
        ...
        try {
            ...
            const renderNames = [
                ...
                "uCamDist",
                "uPointScale",
                // An array uniform's location is looked up by its first element,
                // and uniform3fv on that location writes the whole run.
                "uColors[0]",
                "uColorCount",
                "uRingPos",
                "uTime",
        ...
        function rebuildField(d: number) {
            ...
            // Seed BOTH targets. Seeding only one means the first swapped frame
            // reads an uninitialised texture and the field flashes from origin.
        ...
        /* ---- pointer: the ONLY interaction. No drag, no camera motion. ---- */
        ...
        const posLocSim = simProg
            ? gl.getAttribLocation(simProg.prog, "aPos")
            : -1
        ...
        const frame = (now: number) => {
            ...
            if (!count) return
            // Hold the last painted frame rather than clearing: a paused field
            // should look still, not empty.
            if (L.paused || !onScreen || document.hidden) {
                last = 0
                return
            }
            ...
            /* -- ring target. The camera is fixed on +Z looking at the origin,
                  ...
                  half-height at the plane is camDist / projF. The source's 0.175
                  gain is applied on top, which is why the ring trails the
                  pointer slightly toward centre. -- */
            ...
            gl.uniform1f(renderProg.u.uOpacity, L.opacity)
